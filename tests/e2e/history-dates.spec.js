...
test("finishing yesterday's game after NZ midnight records under yesterday's date; today's completion records separately", async ({ page }) => {
  ...
  expect(await history(page)).toEqual([
    { date: "2026-7-10", game: "sonar", tier: 1, metrics: { pings: 7, hintsUsed: 0, win: true }, snapshot: expect.any(Object) }
  ]);
  ...
  expect(await history(page)).toEqual([
    { date: "2026-7-10", game: "sonar", tier: 1, metrics: { pings: 7, hintsUsed: 0, win: true }, snapshot: expect.any(Object) },
    { date: "2026-7-11", game: "sonar", tier: 1, metrics: { pings: 7, hintsUsed: 0, win: true }, snapshot: expect.any(Object) }
  ]);
