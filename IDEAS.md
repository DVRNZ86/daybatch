...
## Stage D — monetization & go-to-market (not started; see PLAN.md A8)

- **Monetization model + marketing plan** — Darren's original brief (before this repo existed) asked for a plan to "implement, market and monetise." Stage B deliberately ships zero monetization (payments/accounts/premium tier are all "Out of Stage B"), so this is still outstanding. Two premium-mode seeds already exist above (Endless Crossing, Codebreak: Repeats) as the likely freemium wedge. Needs its own dedicated discussion once B4/B5 ship — anchor a go-to-market timeline on the 10 Sep 2026 launch (EPOCH). (Filed 12 Jul 2026.)
- **Web-only vs. native wrap (Capacitor)** — open question, not a decision. Staying a pure PWA means monetization is limited to web-native payments (e.g. Stripe payment links, no accounts) — no native App/Play Store IAP. A future Capacitor wrap would unlock native IAP + store discovery but is a real architecture shift, currently out of Stage B scope entirely. Revisit alongside the monetization model discussion above; don't decide by default. (Filed 12 Jul 2026, see PLAN.md A8.)
