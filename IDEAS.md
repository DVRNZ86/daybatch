...
- **Web-only vs. native wrap (Capacitor)** — open question, not a decision. Staying a pure PWA means monetization is limited to web-native payments (Stripe, no accounts) — no native App/Play Store IAP. A future Capacitor wrap would unlock native IAP + store discovery but is a real architecture shift, currently out of Stage B scope entirely. Revisit only if D1's web-only model hits a real ceiling. (Filed 12 Jul 2026, see PLAN.md A9.)
